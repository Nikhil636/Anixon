...
class _topmoviesState extends State<topmovies> {
  ...
  Future<void> fetchData() async {
    try {
      final topAnimeData = await ApiService.fetchTopMoviesAnimeData();
      setState(() {
        topMovieList = topAnimeData;
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: topMovieList.isEmpty
          ? Center(child: CircularProgressIndicator())
          : ListView.builder(
              physics: ClampingScrollPhysics(),
              itemCount: topMovieList.length > 50 ? 50 : topMovieList.length,
              itemBuilder: (context, index) {
                Map<String, dynamic> animeData = topMovieList[index];
                return Container(
