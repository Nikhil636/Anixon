...
class favoriteanime extends StatefulWidget {
  const favoriteanime({Key? key}) : super(key: key);
...
class _favoriteanimeState extends State<favoriteanime> {
  ...
  Future<void> fetchData() async {
    try {
      ...
      setState(() {
        MostFavoriteAnimeList = topAnimeData;
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: MostFavoriteAnimeList.isEmpty
          ? Center(child: CircularProgressIndicator())
          : ListView.builder(
              physics: ClampingScrollPhysics(),
              itemCount: MostFavoriteAnimeList.length > 50
                  ? 50
                  : MostFavoriteAnimeList.length,
              itemBuilder: (context, index) {
                Map<String, dynamic> animeData = MostFavoriteAnimeList[index];
                return Container(
